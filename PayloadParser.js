function parseUplink(device, payload)
{
    ...
    temperature1 = data["data"]["temperature1"];
    temperature2 = data["data"]["temperature2"];
    vin = data["data"]["vin"];
    vbatt = data["data"]["vbatt"];
    modemSignal = data["data"]["modemSignal"];

    device.endpoints.byAddress("1").updateTemperatureSensorStatus(temperature1, timestamp);
    device.endpoints.byAddress("2").updateTemperatureSensorStatus(temperature2, timestamp);
    device.endpoints.byAddress("5").updateVoltageSensorStatus(vin, timestamp);
    device.updateDeviceRssi({ quality: modemSignal });	
    device.updateDeviceBattery({ voltage: vbatt, state: batteryState.low });
/*  
